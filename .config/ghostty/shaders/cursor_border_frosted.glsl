...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float outerGlowWidth = borderWidth * 5.0;
    ...
    vec3 frostTint = vec3(0.50, 0.98, 1.0);
    vec3 frostDeep = vec3(0.0, 0.18, 1.0);

    float grain = noise(fragCoord * 0.75 + iTime * 8.0) - 0.5;
    vec3 frosted = mix(blurred.rgb, frostTint, 0.28) + grain * 0.035;

    vec3 color = base.rgb;

    color = mix(color, frosted, innerFrost * 0.42);
    color = mix(color, frostTint, outerGlow * 0.18);

    vec3 borderColor = mix(frostTint, frostDeep, 0.22);
    color = mix(color, borderColor, border * 0.82);
